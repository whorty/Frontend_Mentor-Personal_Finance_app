...
import PotAmountModal from "../modals/PotAmountModal";
import { useState } from "react";

export default function PotsCards(props: Total) {
  const [openAmountM, setOpenAmountM] = useState(false);
  const [mode, setMode] = useState<"Add" | "withdraw">();
  function OpenModal(selectedMode: "Add" | "withdraw" | undefined) {
    setMode(selectedMode);
    setOpenAmountM(!openAmountM);
  }
  return (
    <>
      {openAmountM && (
        <PotAmountModal {...props} ToggleModal={OpenModal} mode={mode} />
      )}
      <div className="cardinfo bg-white">
        ...
        <div className="potCard-options">
          <button className="btn bg-grey" onClick={() => OpenModal("Add")}>
            <strong>+ Add Money</strong>
          </button>
          <button className="btn bg-grey" onClick={() => OpenModal("withdraw")}>
